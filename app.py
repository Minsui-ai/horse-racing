...
if not df_trend.empty:
    ...
    with tabs[0]: # 라이징 테마
        ...
        latest = df_trend['date'].max()
        this_w = df_trend[(df_trend['date'] > (latest - timedelta(days=7))) & (df_trend['gender'] == '전체')]
        prev_w = df_trend[(df_trend['date'] <= (latest - timedelta(days=7))) & (df_trend['date'] > (latest - timedelta(days=14))) & (df_trend['gender'] == '전체')]
    ...
    with tabs[1]: # 트렌드 비교 (차트 1-3)
        ...
        fig1 = px.line(df_trend[df_trend['gender'] == '전체'], x='date', y='ratio', color='keyword', line_dash='age_group', title="[그래프 01] 키워드-연령별 통합 시나리오")
        st.plotly_chart(fig1, use_container_width=True)
        st.info("💡 **분석 의견**: 이 시계열 차트는 각 키워드와 연령대의 검색 비중을 독립적으로 보여줍니다. 특정 시점의 상승은 단기적인 마케팅 이슈나 시즌성 수요를 반영하며, 선의 기울기를 통해 시장의 성장 모멘텀을 확인할 수 있습니다. (50자 이상)")

        st.subheader("2. 성별 검색 트렌드 비교 (Gender Focus)")
        gender_kw = st.selectbox("성별 분석 키워드", keywords)
        fig2 = px.line(df_trend[(df_trend['keyword'] == gender_kw) & (df_trend['gender'] != '전체')], x='date', y='ratio', color='gender', title=f"[그래프 02] '{gender_kw}' 남녀 검색량 추이")
        st.plotly_chart(fig2, use_container_width=True)
        st.info("💡 **분석 의견**: 남녀별 검색 성향 차이를 분석합니다. 특정 성별의 지배적 상승은 해당 키워드가 가지는 성별 타겟 특성을 명확히 드러내며, 이를 기반으로 마케팅 채널 믹스(Mix)를 조정해야 합니다. (50자 이상)")

        st.subheader("3. 요일별 열지도 (Activity Heatmap)")
        df_trend['day'] = df_trend['date'].dt.day_name()
        h_pivot = df_trend[df_trend['gender'] == '전체'].pivot_table(index='keyword', columns='day', values='ratio', aggfunc='mean')
        h_pivot = h_pivot.reindex(columns=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"])
    ...
    with tabs[2]: # 세그먼트 분석 (차트 4-7)
        ...
        with col_s1:
            ...
            age_data = df_trend[df_trend['gender'] == '전체'].groupby('age_group')['ratio'].mean().reset_index()
            fig4 = px.pie(age_data, values='ratio', names='age_group', hole=0.4, title="[그래프 04] 연령별 관심도 점유율")
            st.plotly_chart(fig4, use_container_width=True)
        with col_s2:
            st.subheader("5. 성별 시장 참여 비중")
            gen_data = df_trend[df_trend['gender'] != '전체'].groupby('gender')['ratio'].mean().reset_index()
        ...
        fig6 = px.treemap(df_trend[df_trend['gender'] != '전체'], path=['keyword', 'gender', 'age_group'], values='ratio', title="[그래프 06] 시장 세분화 구조 (키워드/성별/연령)")
        st.plotly_chart(fig6, use_container_width=True)
        
        st.subheader("7. 연령대별 변동성 진단 (Volatility Box)")
        fig7 = px.box(df_trend, x='age_group', y='ratio', color='gender', title="[그래프 07] 속성별 데이터 분산 및 이상치 식별")
